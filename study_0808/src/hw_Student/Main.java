package hw_Student;

public class Main {
	public static void main(String[] args) {

		Student20 student201 = new Student20("A", 23, "국문과");
		Student20 student202 = new Student20("B", 23, "국문과");		
		Student20 student203 = new Student20("C", 24, "수학과");
		Student20 student204 = new Student20("A", 23, "국문과");
		Student20 student205 = new Student20("B", 23, "경영학과");		
		Student20 student206 = new Student20("C", 24, "수학과");
		Student20 student207 = new Student20("A", 23, "국문과");
		Student20 student208 = new Student20("B", 23, "무용과");		
		Student20 student209 = new Student20("C", 24, "수학과");
		Student20 student2010 = new Student20("A", 23, "국문과");
		Student20 student2011 = new Student20("B", 23, "국문과");		
		Student20 student2012 = new Student20("C", 24, "수학과");
		Student20 student2013 = new Student20("A", 23, "유아교육과");
		Student20 student2014 = new Student20("B", 23, "컴공과");		
		Student20 student2015 = new Student20("C", 24, "법학과");
		
		student201.studentInfo();
		student202.studentInfo();
		student203.studentInfo();
		student204.studentInfo();
		student205.studentInfo();
		student206.studentInfo();
		student207.studentInfo();
		student208.studentInfo();
		student209.studentInfo();
		student2010.studentInfo();
		student2011.studentInfo();
		student2012.studentInfo();
		student2013.studentInfo();
		student2014.studentInfo();
		student2015.studentInfo();
		
		
		//Student22 student221 = new Student22("A", 21, "경영학과");
		//student221.studentInfo();
		
		//student2012.setMajorList();
		
		//System.out.println(student206.majorList.size());
	}
}
